...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                $content = "Bonjour ".$user->getFirstname()."<br/>Vous avez demandé à réinitialiser votre mot de passe sur le site PetsLands.<br/><br/>";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant pour <a href='".$url."'>mettre à jour votre mot de passe</a>.";

                $mail = new Mail();
                $mail->send($user->getEmail(), $user->getFirstname().' '.$user->getLastname(), 'Réinitialiser votre mot de passe sur PetsLands', $content);
    ...
    public function update(Request $request, $token, UserPasswordHasherInterface $passwordHasher) // Utilisez UserPasswordHasherInterface
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $hashedPassword = $passwordHasher->hashPassword($reset_password->getUser(), $new_pwd);
